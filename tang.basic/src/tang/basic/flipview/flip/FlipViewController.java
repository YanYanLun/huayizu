...
import tang.basic.model.FlipViewControllerBean;
...
public class FlipViewController extends AdapterView<Adapter> {
  ...
  public FlipViewController(Context context, AttributeSet attrs, int defStyle) {
    ...
    TypedArray
        a =
        context.getTheme().obtainStyledAttributes(attrs, FlipViewControllerBean.flipViewController, 0, 0);

    try {
      int value = a.getInteger(FlipViewControllerBean.orientation, VERTICAL);
      if (value == HORIZONTAL) {
        orientation = HORIZONTAL;
      }

      value = a.getInteger(FlipViewControllerBean.animationBitmapFormat, 0);
      if (value == 1) {
