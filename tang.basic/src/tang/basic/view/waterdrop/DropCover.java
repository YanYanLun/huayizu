...
public class DropCover extends SurfaceView implements SurfaceHolder.Callback {
	...
	private void drawBezier(Canvas canvas) {
		...
		Point[] points = calculate(new Point(mBaseX, mBaseY), new Point(
				mTargetX + mDest.getWidth() / 2f, mTargetY + mDest.getHeight()
						/ 2f));
		...
		path1.quadTo((points[2].x + points[3].x) / 2,
				(points[2].y + points[3].y) / 2, points[1].x, points[1].y);
		canvas.drawPath(path1, mPaint);

		Path path2 = new Path();
		path2.moveTo(points[2].x, points[2].y);
		path2.quadTo((points[0].x + points[1].x) / 2,
				(points[0].y + points[1].y) / 2, points[3].x, points[3].y);
	...
	private Point[] calculate(Point start, Point end) {
		...
		float x = (float) Math.sqrt(a * a / (a * a + b * b)
				* (mStrokeWidth / 2f) * (mStrokeWidth / 2f));
		float y = -b / a * x;
	...
	public void surfaceDestroyed(SurfaceHolder holder) {
		...
		// Toast.makeText(getContext(), "大哥我爆炸了", Toast.LENGTH_SHORT).show();
	}
