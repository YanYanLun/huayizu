...
public class ViewAnimator<T extends Resourceble> {
	...
	public ViewAnimator(ActionBarActivity activity, List<T> items,
			ScreenShotable screenShotable, final DrawerLayout drawerLayout,
		...
		if (list != null) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (StringUtil.isEmpty(item.getTitle())) {
					viewMenu = actionBarActivity.getLayoutInflater().inflate(
							R.layout.menu_list_item, null);
				} else {
					viewMenu = actionBarActivity.getLayoutInflater().inflate(
							R.layout.menu_list_item2, null);
				}
				final int finalI = i;
				viewMenu.setTag(item);
				viewMenu.setOnClickListener(new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						int[] location = { 0, 0 };
						v.getLocationOnScreen(location);
						switchItem(list.get(finalI),
								location[1] + v.getHeight() / 2,
								(SlideMenuItem) v.getTag());
					}
				});
				...
				if (!StringUtil.isEmpty(item.getTitle())) {
					((TextView) viewMenu.findViewById(R.id.title)).setText(item
							.getTitle());
					;
				}
	...
	private void animateView(int position) {
		...
		FlipAnimation rotation = new FlipAnimation(90, 0, 0.0f,
				view.getHeight() / 2.0f);
	...
	private void animateHideView(final int position) {
		final View view = viewList.get(position);
		FlipAnimation rotation = new FlipAnimation(0, 90, 0.0f,
				view.getHeight() / 2.0f);
		rotation.setDuration(ANIMATION_DURATION);
		rotation.setFillAfter(true);
		rotation.setInterpolator(new AccelerateInterpolator());
