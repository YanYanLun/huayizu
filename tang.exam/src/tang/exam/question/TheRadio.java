...
public class TheRadio {
	...
	public TheRadio(Activity context, LinearLayout layout, Exam question, List<AnswerQuestionnaire> answerList,
			int index) {
	...
	public void Start() {
		...
		for (int j = 0; j < optionList.length; j++) {
			...
			if (!StringUtil.isEmpty(mQuestion.MemberChoice)) {
				// 将答案转为大写
				if (Letter[j].equals(mQuestion.MemberChoice.toUpperCase())) {
						...
						if (Integer.parseInt(v.getTag().toString()) == k) {
							...
							for (AnswerQuestionnaire item : answerList) {
								if (item.QuestionnaireItemId == Integer.parseInt(title.getTag().toString())) {
									item.Answer = serialNumber.getText().toString();
								}
							}
							// 给已经答过的题目改为绿色
